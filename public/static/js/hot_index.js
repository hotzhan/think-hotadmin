 ...
 */
$(function () {
    //登录时，将当前链接作为redirect参数传入
    $('.login').on('click', function () {
        var redirect = window.location.href
        var url = $(this).data('url');
        //url += (url.indexOf("?") === -1 ? "?" : "&")  + 'redirect=' + redirect;
...
/** 跳转到指定url */
function goUrl(url, ispjax= false) {
    console.log('链接' + url)
 ...
 * @returns {string}
 */
function generateIndexMenuTree(data, paddingLeft=0) {
    var html = '';
    var noPjax = '';
    paddingLeft += 8;//子菜单缩进
    // 遍历每个节点
    ...
    for(var i in data) {
        if($.isEmptyObject(data[i].children))
        {
            if(data[i].parent_id == 1)
            {
                ...
                html += '<a href="' + data[i].url + '" class="nav-link text-nowrap">' + data[i].name + '</a>';
                html += '</li>';
            }
            else
            {
        ...
        else
        {
            if(data[i].parent_id == 1)
            {
                ...
                html += '<a id="dropdownSubMenu"'+data[i].id+' href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link dropdown-toggle text-nowrap">' + data[i].name + '</a>';

            }

            else
            {
                html += '<li class="dropdown-submenu dropdown-hover">';
                html += '<a id="dropdownSubMenu"'+data[i].id+' href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="dropdown-item dropdown-toggle">' + data[i].name + '</a>';

            }

            html += '<ul aria-labelledby="dropdownSubMenu"'+data[i].id+' class="dropdown-menu border-0 shadow">';
            html += generateIndexMenuTree(data[i].children, paddingLeft) ;
            html += '</ul>';
        }
    }
    //html += '</ul>';
